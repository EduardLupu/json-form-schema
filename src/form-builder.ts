...
type FieldType = 'text' | 'email' | 'password' | 'number' | 'textarea' | 'select' | 'checkbox' | 'radio' | 'hidden' | 'telephone' | 'date' | 'color' | 'file' | 'time' | 'image';
...
const countries: { name: string; code: string; prefix: string }[] = [
...
type ThemeStyle = 'modern' | 'minimal' | 'rounded' | 'flat' | 'outlined';
type ThemeColor = 'ocean' | 'sunset' | 'forest' | 'cosmic' | 'solarized';
...
export class FormBuilder extends HTMLElement {
  ...
  private searchableSelects: Map<string, HTMLInputElement> = new Map();
  private currentTheme: Theme = { style: 'modern', color: 'ocean' };
  ...
  private createField(field: FormField): HTMLElement {
    ...
    switch (field.type) {
      case 'textarea':
        ...
        if (field.rows) input.setAttribute('rows', field.rows.toString());
        if (field.cols) input.setAttribute('cols', field.cols.toString());
        break;
      ...
      case 'select':
        if (field.searchable) {
          ...
          if (field.defaultValue !== undefined) {
            ...
            const defaultOption = field.options?.find(opt => 
          ...
          if (field.multiple) input.setAttribute('multiple', '');
          if (field.options) {
            field.options.forEach(option => {
      ...
      case 'telephone':
        ...
        optionsContainer.addEventListener('click', (e) => {
          const option = (e.target as HTMLElement).closest('.country-option') as HTMLElement;
          if (option) {
        ...
        document.addEventListener('click', (e) => {
          if (!countryCodeWrapper.contains(e.target as Node)) {
            countryOptions.style.display = 'none';
          }
        });
      ...
      default:
        ...
        if (field.min !== undefined) input.setAttribute('min', field.min.toString());
        if (field.max !== undefined) input.setAttribute('max', field.max.toString());
        if (field.step !== undefined) input.setAttribute('step', field.step.toString());
      ...
      if (field.defaultValue !== undefined) {
        if (input instanceof HTMLInputElement) {
          ...
          if (field.type === 'checkbox') {
            input.checked = field.defaultValue as boolean;
          } else {
        ...
        Object.entries(field.style).forEach(([key, value]) => {
          if (key !== 'customClass' && value) {
            ...
            if (styleKey in input.style) {
              (input.style as any)[styleKey] = value;
            }
          }
        });
      }
  ...
  private handleSubmit(event: Event) {
    ...
    for (const [name, value] of formData.entries()) {
      ...
      if (field?.type === 'checkbox') {
        // @ts-ignore
        values[name as keyof FormValues] = (event.target as HTMLFormElement).elements[name]?.checked || false;
      } else if (field?.type === 'telephone') {
        // Get the country code prefix
        const countryCodeInput = formData.get(`${name}_country_code`) as string;
        ...
        if (country) {
          ...
          values[name as keyof FormValues] = `+${country.prefix}${number}`;
        } else {
          values[name as keyof FormValues] = value.toString();
        }
      } else {
        values[name as keyof FormValues] = value.toString();
      }
  ...
  public getValues(): FormValues {
    ...
    for (const [name, value] of formData.entries()) {
      ...
      if (field?.type === 'checkbox') {
        // @ts-ignore
        values[name as keyof FormValues] = (this.form.elements[name] as HTMLInputElement).checked;
      } else {
        values[name as keyof FormValues] = value.toString();
      }
    }
    ...
    this.searchableSelects.forEach((input, name) => {
      const hiddenInput = input.closest('.select-wrapper')?.querySelector(`input[name="${name}"]`) as HTMLInputElement;
      if (hiddenInput) {
        values[name as keyof FormValues] = hiddenInput.value;
      }
    });
  ...
  set values(newValues: FormValues) {
    ...
    this.schema.forEach(field => {
      // @ts-ignore
      const input = this.form.elements[field.name] as HTMLInputElement;
      if (input) {
        if (field.type === 'checkbox') {
          input.checked = newValues[field.name as keyof FormValues] as boolean;
        } else {
          input.value = newValues[field.name as keyof FormValues]?.toString() || '';
        }
      }
    });
  }
  ...
  attributeChangedCallback(name: string, oldValue: string, newValue: string) {
    if (name === 'font-family') {
      ...
      try {
        ...
        this.schema = Array.isArray(this.schema) ? this.schema : (this.schema as FormSchema).fields;
      ...
      this.currentTheme.style = newValue as ThemeStyle;
      this.updateTheme();
    } else if (name === 'theme-color') {
      this.currentTheme.color = newValue as ThemeColor;
      this.updateTheme();
    }
  }

  private async loadSchemaFromUrl(url: string) {
    try {
